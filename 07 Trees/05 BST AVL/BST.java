...
public class BST implements BSTinterface
{
   ...
   */
   ...
   {
      add(value);
      balanceTree(  root   );   // for an AVL tree. Put in the arguments you want.
   }
   private void balanceTree( TreeNode current  )  //recursive.  Whatever makes sense.
   {
     if(current == null)
     return;
     if(current.getLeft() != null)
     balanceTree(current.getLeft());
     if(current.getRight() != null)
     balanceTree(current.getRight());
             
          System.out.println(calcBalance(current));    
     if(calcBalance(current) == 2) //right heavy
     {
      if(calcBalance(current.getRight()) == -1)
      {
      ...
      current = LeftLeft(current); 
      }
      
     }
     else if(calcBalance(current) == -2) //left heavy
     {
    
      if(calcBalance(current.getLeft()) == 1)
      {
   ...
   // 4 rotation methods
   TreeNode RightRight (TreeNode x)
   {
   ...
   TreeNode LeftLeft (TreeNode x)
   {  
      return x;
   }
   TreeNode LeftRight(TreeNode x)
   {
    
      return x;
   }
   TreeNode RightLeft(TreeNode x)
   {
