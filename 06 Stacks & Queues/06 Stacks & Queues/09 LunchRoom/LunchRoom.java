...
public class LunchRoom
{
   ...
   public static int Freshmen = 0;
   public static int Sophmores = 0;
   public static int Juniors = 0;
   public static int Seniors = 0;
   public static int Frwait = 0;
   public static int Sowait = 0;
   public static int Juwait = 0;
   public static int Srwait = 0;
   public static double Frwaitavg = 0;
   public static double Sowaitavg = 0;
   public static double Juwaitavg = 0;
   public static double Srwaitavg = 0;
   ...
   public static void lunchRoom(int TIME, PrintWriter outfile)
   {
      ...
      *************************/
       ...
       for(int x = TIME; x > 0; x--){
        ...
        if(customerList.size() != 0){
           //waittime
           if(customerList.peek().getGradyear().equals("Fr"))
              {
              Freshmen++;
              }
              if(customerList.peek().getGradyear().equals("So"))
              {
              Sophmores++;
              }
              if(customerList.peek().getGradyear().equals("Ju"))
              {
              Juniors++;
              }
              if(customerList.peek().getGradyear().equals("Sr"))
              {
              Seniors++;
              }
           ...
           //total
              ...
              if(customerList.peek().getGradyear().equals("Fr"))
              {
              if(customerList.peek().getTotalWait()>Frwait)
              Frwait = customerList.peek().getTotalWait();
              Frwaitavg = customerList.peek().getTotalWait();


              }
              if(customerList.peek().getGradyear().equals("So"))
              {
              if(customerList.peek().getTotalWait()>Sowait)
              Sowait= customerList.peek().getTotalWait();
              Sowaitavg = customerList.peek().getTotalWait();

              }
              if(customerList.peek().getGradyear().equals("Ju"))
              {
              if(customerList.peek().getTotalWait()>Juwait)
              Juwait = customerList.peek().getTotalWait();
              Juwaitavg = customerList.peek().getTotalWait();

              }
              if(customerList.peek().getGradyear().equals("Sr"))
              {
              if(customerList.peek().getTotalWait()>Srwait)
              Srwait = customerList.peek().getTotalWait();
              Srwaitavg = customerList.peek().getTotalWait();
     ...
     while(customerList.size() != 0){
      ...
      System.out.println("Customer\t\tTotal\t\tLongest\t\tAverage Wait");
       System.out.println("Senior\t\t" + Seniors +"\t\t" + Srwait +"\t\t" + Srwaitavg);
       System.out.println("Juniors\t\t" + Juniors +"\t\t" + Juwait + "\t\t" + Juwaitavg);
       System.out.println("Sophmores\t\t" + Sophmores +"\t\t" + Sowait +"\t\t" + Sowaitavg);
       System.out.println("Freshmen\t\t" + Freshmen +"\t\t" + Frwait + "\t\t" + Frwaitavg);
  ...
  public String toString(){
     return customerNum + ":" + gradyear;
  }
  public int compareTo(Customer student)
  {
  int FirstCustomerYear = 0;
  int SecondCustomerYear = 0;
  //FirstCustomer
  if(gradyear.equals("Sr"));
  FirstCustomerYear = 1;
  if(gradyear.equals("Ju"));
  FirstCustomerYear = 2;
  if(gradyear.equals("So"));
  FirstCustomerYear = 3;
 if(gradyear.equals("Fr"));
  FirstCustomerYear = 4;
//Second Customer
  if(student.getGradyear().equals("Sr"))
  SecondCustomerYear = 1;
  if(student.getGradyear().equals("Ju"))
  SecondCustomerYear = 2;
  if(student.getGradyear().equals("So"))
  SecondCustomerYear = 3;
  if(student.getGradyear().equals("Fr"))
  SecondCustomerYear = 4;
  
  
  if(SecondCustomerYear > FirstCustomerYear)
  {
  return -1;
  }
  else if(SecondCustomerYear == FirstCustomerYear)
  {
  return 0;
